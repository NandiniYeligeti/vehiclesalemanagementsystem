...
import { X, Upload, FileText, Check, Plus, Loader2, Download } from 'lucide-react';
...
const CustomerProfile: React.FC<CustomerProfileProps> = ({ customer, onClose, mode = 'view' }) => {
  ...
  // Filter for this customer (with safety checks)
  ...
  const handleUploadDocs = () => {
    ...
    setIsUpdating(true);
    // In a real app, this would be a multi-part form data upload
      ...
      <div className="bg-card rounded-2xl ring-1 ring-border shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-border bg-muted/20">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-[10px] bg-primary/10 text-primary font-black uppercase px-2 py-0.5 rounded-full tracking-tighter">
                  {customer._id?.slice(-6).toUpperCase() || 'CUST'}
                </span>
          ...
          <div className="flex items-center gap-2">
            {mode === 'edit' && newDocuments.length > 0 && (
                      ...
                      <div key={so._id || so.id} className="erp-card p-5 group hover:border-primary/40 transition-all">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-[11px] font-medium border-t border-border/50 pt-4">
                          <div>
                            ...
                            <span className="font-mono text-foreground/80">{so.chassis_number || 'LOADING...'}</span>
                          </div>
                          <div>
                      ...
                      <tbody>
                        {customerPayments.map((p: any) => (
                          <tr key={p._id || p.id} className="border-b border-border/50 hover:bg-muted/10">
                            <td className="px-6 py-4 font-mono font-bold">{p.reference_number || 'REC-998'}</td>
                            ...
                            <td className="px-6 py-4 text-right font-black text-emerald-600">₹{(p.amount || 0).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    ...
                    <div className="bg-primary/5 border border-primary/20 rounded-2xl p-6">
                      ...
                      {newDocuments.length > 0 && (
                        <div className="mt-6 space-y-2">
                          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60">To be uploaded:</p>
                          {newDocuments.map((f, i) => (
                            <div key={i} className="flex items-center justify-between p-3 bg-white rounded-xl border border-primary/20 shadow-sm animate-in slide-in-from-left-2 duration-300">
